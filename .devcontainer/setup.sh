...
# banco, código do Moodle, site instalado, verificador de PHPDoc, dependências de build
# e as ferramentas de bancada. O boot de cada dia é o start.sh (leve).
set -euo pipefail
...
CLI="$MOODLE_DIR/public/admin/cli"

echo "==> [1/6] MariaDB — serviço e banco"
...
echo "==> [2/6] Código do Moodle ($MOODLE_BRANCH)"
...
echo "==> [3/6] local_moodlecheck (verificador de PHPDoc, o mesmo do CI)"
...
echo "==> [4/6] Instalação do site Moodle"
if [ ! -f "$MOODLE_DIR/config.php" ]; then
    ...
    php "$CLI/install_language_pack.php" pt_br || true
    ...
    if ! grep -q 'phpunit_prefix' "$MOODLE_DIR/config.php"; then
        # Insere as linhas de phpunit ANTES do require final do setup.php.
...
echo "==> [5/6] Dependências de build do Moodle (grunt, eslint, stylelint)"
if [ ! -d "$MOODLE_DIR/node_modules" ]; then
    ( cd "$MOODLE_DIR" && npm install --no-audit --no-fund --loglevel=error )
fi

echo "==> [6/6] Ferramentas de bancada (moodle-dev-tools — conjunto curado)"
